...
const RepeaterGridHeader: React.FC<{ element: UIElement }> = ({ element }) => {
  ...
  return (
    ...
    //   className={`BgGrid card col-md-12 ${element.ColumnCss ?? ""} ${
    //     element.Css ?? ""
    //   }`.trim()}
    //   style={{ fontStyle: "normal", textDecoration: "none" }}
    // >
      <div className={`card-header px-3 py-3 d-flex flex-wrap justify-content-between align-items-center ${element.ColumnCss ?? ""} ${
        element.Css ?? ""
      }`} 
      >
        <div className="d-flex align-items-center gap-2 mb-2 mb-md-0">
          <h6 className="fw-semibold text-purple mb-0">
            {element.DisplayName || element.ElementName.split("_")[1] || "Grid Header"}
          </h6>
