...
export const extractDocumentSettings = (documentSettings: string): IDocumentSettings => {
  try {
    const settings = JSON.parse(documentSettings) as IDocumentSettings[];

    const initialSettings = settings[0];

    return {
      Size: initialSettings.Size ? initialSettings.Size * 1024 * 1024 : 10 * 1024 * 1024,
      AllowedTypes: initialSettings.AllowedTypes ? initialSettings.AllowedTypes : "*",
    }
  } catch (error) {
    console.error("Error extracting document settings:", error);
    return {
